...
try {
  if (!org || !user || !file || !output || extra.length)
    throw new DomainError(
      'USAGE',
      'Usage: npm run migration:preview:tariffs -- <organisation-id> <verified-owner-or-admin-id> <bundle.json> <new-report.json>',
    );
  uuid.parse(org);
  uuid.parse(user);
  const input = await readMigrationJson(file);
  const service = new LegacyTariffPreflight(
    db,
    {
      async send() {
  ...
  console.log(
    `Dry run: ${report.ready ? 'ready for review' : 'blocked'}; ${report.issues.length} source issues; ${report.targetConflicts.length} target conflicts. No database records changed.`,
  );
