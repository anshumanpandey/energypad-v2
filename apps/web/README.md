...
Validate and review the preview before committing. Download CSV errors to fix invalid rows. Each commit is atomic and retry-safe; reopening or changing mapping requires revalidation. Existing periods are rejected rather than overwritten. The importer discards credential columns and rejects macros/external links under the same limits as site imports.
...
All XLSX imports accept styled cells, rich text, hyperlink labels, and saved formula results (including shared formulas). Numeric values retain their underlying precision: currency symbols, grouping separators and display rounding are not imported. Percentage formats do not change the value: a cell storing `0.2` and displaying `20%` imports as `0.2`; fields such as VAT percentage still expect `20`.

Excel date cells import as `YYYY-MM-DD`; date cells formatted with year/month and no day or time import as `YYYY-MM`. Formula results follow the same rules. The importer reads the result saved by Excel; it does not recalculate formulas. Recalculate and save before uploading. Missing results and Excel errors are rejected with the sheet and cell address. Macros, external workbook links and existing size/dimension limits remain blocked.

### Regression workbook compatibility

Run `npm run test:compatibility -- /path/to/reference-workbooks /path/to/new-report.json` to test the three private Sprint 4 references. It verifies their hashes and exact registered output cells, recomputes results, applies the approved absolute tolerance of 0.99, and requires exact significance decisions. Exit 0 is numerical PASS, 1 is failure/error, and 2 is blocked reference coverage. No workbook is changed or downloaded; reports are private and never overwritten. See [the workbook review](../../docs/SPRINT_4_WORKBOOK_REVIEW.md) for scope and the distinction from full methodological acceptance.

### Carbon emission factors

The Carbon page supports organisation-owned emission factors in kgCO2e/kWh, with fuel, geography, reporting basis, source and effective dates. Owners/admins can add and correct factors; active members can inspect history. Apply migrations before use. Corrections preserve original versions; overlapping current factors for the same identity are rejected. The Carbon page also saves calendar-year calculations per meter, geography and reporting basis. Missing months or incomplete factor coverage block annual totals. Snapshots preserve exact input versions after corrections. Run `node --import tsx scripts/emission-factors-integration.ts` for isolated database checks or `npx playwright test tests/e2e/emission-factors.spec.ts` for the browser flow.

Run `node --import tsx scripts/carbon-integration.ts` for carbon calculation, history and access-control checks.

Site carbon summaries are available on the Carbon page and `GET /api/v1/organisations/:org/sites/:site/carbon/summary` with `year`, `geography` and `basis` query parameters. They sum the latest matching runs across currently active meters only when all have complete, current source coverage; missing, blocked or outdated runs withhold the total. Summaries retain run references and expose estimated-month counts. The result is a meter sum, without deductions for overlapping main/submeter measurements. Recheck after input changes.

Annual carbon targets are available beneath the carbon calculation form. Define an absolute kgCO2e limit per meter/year/geography/basis, then assess a complete matching saved run. Corrections create new target revisions; saved assessments preserve the old target/run pairing. The assessment shows historical MET/EXCEEDED status and actual-minus-limit variance; use the site summary to check current input coverage. Apply migrations before use. Target and assessment routes live under the site's `/carbon/targets` endpoint and are included in `scripts/carbon-integration.ts` and the Carbon browser test.

The Carbon page now imports emission factors and annual carbon targets from XLSX: choose the destination, worksheet and optional version 1 JSON mapping; review every row, confirm and commit. Factors use kgCO2e/kWh and organisation scope; targets use kgCO2e and registered meter codes in the selected site. `supersedesId` plus `reason` appends a correction. Up to 120 rows commit together with a retained row-to-record receipt. Repeated committed uploads reuse the receipt; stale conflicts roll back the entire batch. Run `node --import tsx scripts/carbon-imports-integration.ts` for isolated import checks. Apply the latest migrations before use.
