...
import { eventInput, eventCorrection, mapEventWorkbook, type EventPreview } from '../domain/events';
import { readWorkbook } from './workbook';
const json = (v: unknown) => JSON.parse(JSON.stringify(v));
export class EventService extends FoundationService {
  ...
  async list(actor: Actor, org: string, siteId: string, year: number) {
    ...
    return this.db.operationalEvent.findMany({
      where: {
        organisationId: org,
        siteId,
        replacement: { is: null },
  ...
  private async prepare(
    ...
    if (
      !supersedesId &&
      (await tx.operationalEvent.findFirst({ where: { organisationId: org, siteId, eventCode: data.eventCode } }))
    )
      throw new DomainError(
        'EVENT_CONFLICT',
        'This event code is already registered. Correct the existing event instead.',
        409,
      );
    if (
      !supersedesId &&
      data.legacyId &&
      (await tx.operationalEvent.findFirst({
        where: { organisationId: org, legacySource: data.legacySource, legacyId: data.legacyId },
      }))
    )
      throw new DomainError('LEGACY_CONFLICT', 'This event source identity has already been imported.', 409);
    return record;
  }
  private async insert(
    ...
    const row = await tx.operationalEvent.create({
      data: { ...data, authorId: actor.userId, importBatchId: batchId },
    });
    await this.audit(tx, actor, org, 'event.recorded', row.id, { siteId, ...(batchId ? { batchId } : {}) });
    return row;
  }
  ...
  async correct(actor: Actor, org: string, siteId: string, id: string, input: unknown) {
    const { observation, reason } = eventCorrection.parse(input);
    const wire = observation;
    return this.db.$transaction(async (tx) => {
      await this.access(tx, actor, org, siteId);
      const prior = await tx.operationalEvent.findFirst({
      ...
      if (
        ...
        prior.eventCode !== data.eventCode ||
        prior.legacySource !== data.legacySource ||
        prior.legacyId !== data.legacyId
      )
        throw new DomainError('IDENTITY', 'Keep the event code, end use and legacy identity fixed.');
      const row = await tx.operationalEvent.create({
        data: {
          ...data,
      ...
      await this.audit(tx, actor, org, 'event.corrected', row.id, { siteId, supersedesId: id, reason });
      return row;
    });
  }
  async history(actor: Actor, org: string, siteId: string, id: string) {
    await this.getSite(actor, org, siteId);
    const selected = await this.db.operationalEvent.findFirst({
  ...
  async imports(actor: Actor, org: string, siteId: string) {
    ...
    await this.getSite(actor, org, siteId);
    return this.db.eventImportBatch.findMany({
  ...
  async upload(actor: Actor, org: string, siteId: string, bytes: Uint8Array) {
    ...
    const result = mapEventWorkbook(sheets);
    return this.db.$transaction(
      async (tx) => {
        await this.access(tx, actor, org, siteId);
        const existing = await tx.eventImportBatch.findUnique({
        ...
        for (const row of result.records) {
          try {
            ...
            result.issues.push({ row: row.row, field: 'event', message: error.message });
          }
        }
        const batch = await tx.eventImportBatch.create({
          data: {
            organisationId: org,
            siteId,
            fingerprint,
        ...
        await this.audit(tx, actor, org, 'event.import_previewed', batch.id, {
          siteId,
  ...
  async commit(actor: Actor, org: string, siteId: string, id: string) {
    return this.db.$transaction(
      async (tx) => {
        await this.access(tx, actor, org, siteId);
        const batch = await tx.eventImportBatch.findFirst({
          where: { id: uuid.parse(id), organisationId: org, siteId },
        });
        ...
        const preview = batch.result as unknown as EventPreview;
        ...
        await this.audit(tx, actor, org, 'event.import_committed', batch.id, {
          siteId,
          count: preview.records.length,
        });
        return tx.eventImportBatch.update({
          where: { id },
