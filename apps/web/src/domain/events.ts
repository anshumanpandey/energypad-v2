import { z } from 'zod';
import { correctionReason } from './energy';
import { DomainError } from './policy';
import type { ImportSheet, RowIssue } from './sites';
const date = z
  .string()
  .regex(/^(19|20|21)\d{2}-\d{2}-\d{2}$/)
  .refine((v) => {
    const d = new Date(v);
    return Number.isFinite(+d) && d.toISOString().slice(0, 10) === v;
  }, 'Use a valid YYYY-MM-DD date.');
export const eventInput = z
  .object({
    firstDay: date,
    lastDay: date,
    energyUseCode: z
      .string()
      .trim()
      .toUpperCase()
      .regex(/^[A-Z0-9][A-Z0-9_-]{0,39}$/),
    eventCode: z
      .string()
      .trim()
      .toUpperCase()
      .regex(/^[A-Z0-9][A-Z0-9_-]{0,39}$/),
    operation: z.string().trim().min(1).max(500),
    comments: z.string().max(4000),
    source: z.string().trim().min(3).max(500),
    legacySource: z.string().trim().max(100).default(''),
    legacyId: z.string().trim().max(160).default(''),
  })
  .strict()
  .refine((v) => v.lastDay >= v.firstDay, 'Last day must follow first day.')
  .refine((v) => !v.legacyId || !!v.legacySource, 'A legacy ID requires a source namespace.');
export const eventCorrection = z.object({ observation: eventInput, reason: correctionReason }).strict();
export type EventInput = z.input<typeof eventInput>;
export type EventPreview = {
  sourceRows: number;
  records: { row: number; data: EventInput; energyUseId?: string }[];
  issues: RowIssue[];
};
export function mapEventWorkbook(sheets: ImportSheet[]): EventPreview {
  if (sheets.length !== 1 || !sheets[0].rows.length || sheets[0].rows.length > 240)
    throw new DomainError('EVENT_SHEET', 'Use one sheet with 1–240 event rows.');
  const sheet = sheets[0];
  const fields = [
    'firstDay',
    'lastDay',
    'energyUseCode',
    'eventCode',
    'operation',
    'comments',
    'source',
    'legacySource',
    'legacyId',
  ] as const;
  const headers = sheet.headers.map((h) => h.trim().toLowerCase());
  if (headers.length !== fields.length || fields.some((f) => headers.filter((h) => h === f.toLowerCase()).length !== 1))
    throw new DomainError('EVENT_HEADERS', `Use exactly: ${fields.join(', ')}.`);
  const result: EventPreview = { sourceRows: sheet.rows.length, records: [], issues: [] };
  for (const row of sheet.rows) {
    const raw = Object.fromEntries(fields.map((f) => [f, row.cells[headers.indexOf(f.toLowerCase())] ?? '']));
    const input = raw;
    const parsed = eventInput.safeParse(input);
    if (!parsed.success) {
      result.issues.push({
        row: row.row,
        field: 'event',
        message: 'Invalid dates, event code, operation, comments, end-use code or source.',
      });
      continue;
    }
    const data = parsed.data;
    if (result.records.some((r) => r.data.eventCode === data.eventCode))
      result.issues.push({ row: row.row, field: 'eventCode', message: 'Duplicate event code in the workbook.' });
    if (
      data.legacyId &&
      result.records.some((r) => r.data.legacySource === data.legacySource && r.data.legacyId === data.legacyId)
    )
      result.issues.push({ row: row.row, field: 'legacyId', message: 'Duplicate source identity in the workbook.' });
    result.records.push({ row: row.row, data });
  }
  return result;
}
