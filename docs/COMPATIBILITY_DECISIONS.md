...
| CD12 | Workbook H49:H52 uses strict p<0.01 and p<0.05; V2 text uses ≤0.01 and ≤0.05. | Implemented for the provisional UI in statistical-interpretation-v1: follow the master V2 specification’s inclusive boundaries. Numeric p-values remain unchanged; this does not approve workbook compatibility. | Exactly 0.01/0.05 and adjacent values; Sprint 4. |
...
| CD14 | R² labels differ across references; workbook K41 thresholds are 0.90/0.80/0.75. | Implemented statistical-interpretation-v1 using the available NRA K41 inclusive 0.90/0.80/0.75 thresholds and short verdict labels. Snapshot configuration and verdicts separately from the numerical engine; full-workbook acceptance remains open. | Boundary labels and policy versions; do not silently recompute old report wording. |
...
## Statistical interpretation implementation — 23 September 2026

Audit point 4 is implemented under the request to fix it. CD12 follows the governing V2 specification: p≤0.01 is Very significant; p≤0.05 is Significant; otherwise Not significant. Direct read-only inspection of NRA V2 K41 confirmed ≥0.90, ≥0.80 and ≥0.75. The provisional R² labels are Very strong, Strong, Acceptable and Unreliable, omitting claims that these alone certify reliability or usability. Configuration, source descriptions and resulting labels are frozen with each new baseline under `statistical-interpretation-v1`. Old baselines are not retrospectively interpreted. Undefined/unrepresentable probabilities remain unavailable rather than being classified as zero. These implementation choices do not close the three-workbook/tolerance approval gate.

## Three-workbook discovery update — 23 September 2026

Single and Multi Routine Adjustment V2 are now supplied. The new [workbook review](SPRINT_4_WORKBOOK_REVIEW.md) records all hashes/layouts, 305 numeric comparisons, 36 matching significance flags, cache precision, the multi intercept's cached zero p-value and conflicting R² labels. Earlier statements that two references are missing are historical. CD12/CD14 remain provisional; the source files alone do not supply tolerance or methodology approval. Native Excel recalculation and approved golden compatibility remain open.
